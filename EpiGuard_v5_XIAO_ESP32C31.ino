// ============================================================
//  FALL + SEIZURE DETECTION SYSTEM  v5.0  —  SEEED XIAO ESP32-C3
//  Hardware : XIAO ESP32-C3 + MPU6050 + SSD1306 OLED 0.96"
...
//        XIAO ESP32-C3 fully supports BLE — no hardware issue.
//
//  PORTING NOTES (ESP32-WROOM-32D → XIAO ESP32-C3):
//    1. Pin names changed to Dx notation (XIAO convention):
//         SDA  : GPIO 21 → D4  (GPIO 6)
//         SCL  : GPIO 22 → D5  (GPIO 7)
//         BTN  : GPIO 18 → D3  (GPIO 5)
//         BUZZ : GPIO 25 → D2  (GPIO 4)
//    2. LEDC API: core 3.x (confirmed by compiler — 3.3.8 installed).
//       Uses single-call ledcAttach(pin, freq, res) and pin-based
//       ledcWrite(pin, duty). No channel constant needed.
//    3. LED_BUILTIN removed — not defined on XIAO ESP32-C3 by default.
//    4. Avoided strapping pins updated for ESP32-C3: GPIO 2, 8, 9.
//       (Original ESP32 strapping pins GPIO 0, 5, 12, 15 do not apply.)
//
//  WIRING:
//    MPU6050 SDA   → D4  (GPIO 6)
//    MPU6050 SCL   → D5  (GPIO 7)
//    OLED SDA      → D4  (GPIO 6)   (shared I2C bus with MPU6050)
//    OLED SCL      → D5  (GPIO 7)   (shared I2C bus with MPU6050)
//    OLED VCC      → 3.3V
//    OLED GND      → GND
//    MPU6050 VCC   → 3.3V
//    MPU6050 GND   → GND
//    Button        → D3  (GPIO 5)   (INPUT_PULLUP — LOW = pressed)
//    Buzzer        → D2  (GPIO 4)   (LEDC PWM channel 0)
//
//  OLED I2C ADDRESS: 0x3C (default for most 0.96" SSD1306 modules)
//
//  AVOIDED PINS (ESP32-C3 strapping pins — do not use for peripherals):
//    GPIO 2, 8, 9
//
...
// ===================== PIN DEFINITIONS =====================
// XIAO ESP32-C3 uses Dx pin naming.
// Underlying GPIO numbers shown in comments for reference.
const int PIN_SDA    = D4;   // GPIO 6  — I2C SDA (shared bus)
const int PIN_SCL    = D5;   // GPIO 7  — I2C SCL (shared bus)
const int PIN_BUTTON = D3;   // GPIO 5  — INPUT_PULLUP, LOW = pressed
const int PIN_BUZZER = D2;   // GPIO 4  — LEDC PWM output

// LEDC buzzer — core 3.x single-call API: ledcAttach(pin, freq, res)
// ledcWrite() takes (pin, duty) on core 3.x
...
void setup() {
  ...
  // XIAO ESP32-C3: SDA=D4 (GPIO 6), SCL=D5 (GPIO 7)
  Wire.begin(PIN_SDA, PIN_SCL);

  // GPIO
  pinMode(PIN_BUTTON, INPUT_PULLUP);   // D3 (GPIO 5) — LOW = pressed

  // LEDC buzzer — core 3.x single-call API
...
// ============================================================
//  BUTTON HANDLER — D3 (GPIO 5), INPUT_PULLUP, LOW = pressed
// ============================================================
